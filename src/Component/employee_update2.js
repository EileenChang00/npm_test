...
export default function Employee_update(props){
    ...
    useEffect(()=>{
        const fetchData = async () =>{
            const data = await Promise.all( fetchEmployeeData() );
            console.log(data);
        }
        //setResult(data);
        fetchData();
    ...
    const fetchEmployeeData = () =>{
            const emp = base('employee').find(props.update_id, function(err, record) {
    ...
    const [newEnterDate, setEnterDate] = useState('');
